...
def replace_city_boundary(*, actor: User, name: str, geometry) -> CityBoundary:
    ...
    if len(active) != 1:
        from urbenmend.api.exceptions import Conflict
        raise Conflict("Exactly one active city boundary is required before replacement.", code="BOUNDARY_STATE_INVALID")
    previous = active[0]
    ...
    record_event(actor=actor, action="reference.city_boundary_replaced", target=boundary,
