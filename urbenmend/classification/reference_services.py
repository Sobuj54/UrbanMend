...
from urbenmend.classification.models import Category, CategoryStatus, SeverityKeyword, SeverityKeywordStatus
...
@transaction.atomic
def create_severity_keyword(*, actor: User, term: str, language: str, severity: str, category: str | None = None) -> SeverityKeyword:
    require_role(actor, Role.ADMIN)
    ...
    keyword = SeverityKeyword.objects.create(term=normalized, language=language, severity=severity, category=_keyword_category(category))
    record_event(actor=actor, action="reference.severity_keyword_created", target=keyword,
        after={"term": keyword.term, "language": language, "severity": severity, "category": category, "active": True})
    return keyword

@transaction.atomic
def update_severity_keyword(*, actor: User, keyword_id: int, **changes) -> SeverityKeyword:
    require_role(actor, Role.ADMIN)
    try: keyword = SeverityKeyword.objects.select_for_update().get(pk=keyword_id)
    except SeverityKeyword.DoesNotExist as exc: raise Http404("Severity keyword not found.") from exc
    before = {"term": keyword.term, "language": keyword.language, "severity": keyword.severity,
              "category": keyword.category.slug if keyword.category else None, "active": keyword.status == SeverityKeywordStatus.ACTIVE}
    if "term" in changes:
    ...
    if "category" in changes: keyword.category = _keyword_category(changes.pop("category"))
    if "active" in changes: keyword.status = SeverityKeywordStatus.ACTIVE if changes.pop("active") else SeverityKeywordStatus.RETIRED
    for key, value in changes.items(): setattr(keyword, key, value)
    keyword.save()
    after = {"term": keyword.term, "language": keyword.language, "severity": keyword.severity,
             "category": keyword.category.slug if keyword.category else None, "active": keyword.status == SeverityKeywordStatus.ACTIVE}
    record_event(actor=actor, action="reference.severity_keyword_updated", target=keyword, before=before, after=after)
    return keyword
