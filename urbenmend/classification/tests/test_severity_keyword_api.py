...
def test_duplicate_normalized_term_conflicts() -> None:
    ...
    response=client(AdminFactory()).post(reverse("api:severity-keywords"),
        {"term":" Test  Duplicate ", "language":"bn", "severity":"high"}, format="json")
...
def test_patch_and_delete_update_then_retire_without_hard_delete() -> None:
    ...
    keyword=SeverityKeyword.objects.create(term="temporary api keyword", language="en", severity="low")
    url=reverse("api:severity-keywords-detail", kwargs={"keyword_id":keyword.pk})
    response=client(AdminFactory()).patch(url, {"severity":"medium", "category":category.slug}, format="json")
...
def test_create_requires_core_fields_and_active_category() -> None:
    admin=AdminFactory(); url=reverse("api:severity-keywords")
    assert client(admin).post(url, {"term":"missing fields"}, format="json").status_code == 400
    response=client(admin).post(url, {"term":"unknown category term", "language":"en", "severity":"low", "category":"missing"}, format="json")
