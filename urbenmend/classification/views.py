...
from urbenmend.classification.serializers import SeverityKeywordSerializer, SeverityKeywordWriteSerializer
from urbenmend.classification.reference_services import create_category, update_category, create_severity_keyword, update_severity_keyword
...
class SeverityKeywordCollectionView(APIView):
    ...
    def post(self, request: Request) -> Response:
        serializer = SeverityKeywordWriteSerializer(data=request.data); serializer.is_valid(raise_exception=True)
        required = {"term", "language", "severity"}
        missing = required - serializer.validated_data.keys()
        if missing: raise ValidationError({key: "This field is required." for key in missing})
        keyword = create_severity_keyword(actor=cast("User", request.user), **serializer.validated_data)
...
class SeverityKeywordDetailView(APIView):
    ...
    def patch(self, request: Request, keyword_id: int) -> Response:
        serializer = SeverityKeywordWriteSerializer(data=request.data); serializer.is_valid(raise_exception=True)
        keyword = update_severity_keyword(actor=cast("User", request.user), keyword_id=keyword_id, **serializer.validated_data)
        return Response(SeverityKeywordSerializer(keyword).data)
    def delete(self, request: Request, keyword_id: int) -> Response:
        keyword = update_severity_keyword(actor=cast("User", request.user), keyword_id=keyword_id, active=False)
        return Response(SeverityKeywordSerializer(keyword).data)
