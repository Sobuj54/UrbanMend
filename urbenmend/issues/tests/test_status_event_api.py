...
def test_status_history_is_public_and_has_contract_shape() -> None:
    ...
    actor=AuthorityFactory(); actor.category_scope.add(issue.primary_category)
...
def test_moderated_and_unknown_issues_do_not_leak_history() -> None:
    ...
    import uuid
    assert client().get(reverse("api:issues-status-events", kwargs={"issue_id":uuid.uuid4()})).status_code == 404
